...
1. rᵢ表示一个虚拟寄存器；在tac文本中，**它只能是`%`后接非负整数的格式**，不能任意起名字
2. oᵢ表示一个虚拟寄存器或者一个**整数**常量(o代表operand)
3. lᵢ表示一个标号；在tac文本中，**它只能是`%`后接非负整数的格式**，不能任意起名字
...
stacktrace: 
  - function `main`, line 561, code `call f`, [%0 = 0]
  - function `f`, line 556, code `%6 = (%6 / %6)`, [%0 = 1, %1 = -1610612728("string"), %2 = -572662307(uninitialized), %3 = 233, %4 = 120(ptr), %5 = -1342177257(func<main>), %6 = 0]
```
